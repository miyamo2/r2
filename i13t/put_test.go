...
func TestPutWithPeriod(t *testing.T) {
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestPutWithInterval(t *testing.T) {
	reqTimes := 0
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch reqTimes {
		case 1:
...
func TestPutWithTerminateIf(t *testing.T) {
	t.Parallel()
	reqTimes := 0
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body := TestResponse{
	...
	i := 0
	body := TestRequest{Num: 0}.Encode()
	for res, err := range r2.Put(ctx, ts.URL, body, opts...) {
		Cmp(t, Result{res: res, err: err}, expect[i])
		i++
	}
}
...
func TestPutWithContentType(t *testing.T) {
	...
	i := 0
	body := TestRequest{Num: 0}.Encode()
	for res, err := range r2.Put(ctx, ts.URL, body, r2.WithContentType(r2.ContentTypeApplicationJSON)) {
		Cmp(t, Result{res: res, err: err}, expect[i])
		i++
	}
}
...
func TestPutWithHeader(t *testing.T) {
	...
	i := 0
	body := TestRequest{Num: 0}.Encode()
	for res, err := range r2.Put(ctx, ts.URL, body, r2.WithHeader(http.Header{"X-Test": []string{"test"}})) {
		Cmp(t, Result{res: res, err: err}, expect[i])
		i++
	}
}
...
func TestPutWithAspect(t *testing.T) {
	...
	i := 0
	body := TestRequest{Num: 0}.Encode()
	for res, err := range r2.Put(ctx, ts.URL, body, r2.WithAspect(func(req *http.Request, do func(req *http.Request) (*http.Response, error)) (*http.Response, error) {
...
func TestPutWithAutoCloseResponseBody(t *testing.T) {
	...
	i := 0
	body := TestRequest{Num: 0}.Encode()
	for res, err := range r2.Put(ctx, ts.URL, body, r2.WithAutoCloseResponseBody(false)) {
		Cmp(t, Result{res: res, err: err}, expect[i])
