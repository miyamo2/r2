...
func TestDeleteWithMaxRequestTimes(t *testing.T) {
	...
	i := 0
	body := TestRequest{Num: 0}.Encode()
	for res, err := range r2.Delete(ctx, ts.URL, body, r2.WithMaxRequestTimes(2)) {
		Cmp(t, Result{res: res, err: err}, expect[i])
		i++
	}
}
...
func TestDeleteWithPeriod(t *testing.T) {
	...
	i := 0
	body := TestRequest{Num: 0}.Encode()
	for res, err := range r2.Delete(ctx, ts.URL, body, r2.WithPeriod(10*time.Millisecond), r2.WithMaxRequestTimes(2)) {
		Cmp(t, Result{res: res, err: err}, expect[i])
		i++
	}
}
...
func TestDeleteWithInterval(t *testing.T) {
	...
	i := 0
	body := TestRequest{Num: 0}.Encode()
	for res, err := range r2.Delete(ctx, ts.URL, body, r2.WithInterval(time.Minute), r2.WithMaxRequestTimes(3)) {
		Cmp(t, Result{res: res, err: err}, expect[i])
		i++
	}
}
