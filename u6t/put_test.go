...
func TestPut(t *testing.T) {
	...
	tests := map[string]test{
		...
		"context-cancel": {
			param: param{
				...
				url:     "http://example.com",
				options: []internal.Option{internal.WithNewRequest(stubNewRequest), r2.WithMaxRequestTimes(2), r2.WithInterval(time.Minute)},
				body:    bytes.NewBuffer([]byte(`{"foo": "bar"}`)),
			},
			clientParamResultPairs: []clientParamResultPair{
				{
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			i := 0
			for res, err := range r2.Put(tt.param.ctx, tt.param.url, tt.param.body, append(tt.param.options, r2.WithHttpClient(mockHttpClient))...) {
				if len(tt.wants)-1 < i {
